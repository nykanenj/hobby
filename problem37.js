//Problem 37

const sumAll = () => {
  primesArr.push(2);
  primesArr.push(5);
  let sum = 0;
  primesArr.forEach((prime) => (sum += prime));
  console.log(primesArr.length); // should be 25 if primes less than 100
  console.log(sum); // should be 1060 if primes less than 100
};

const primesArr = [3, 7]; // 5 omitted
let currentSquareRoot = 3;

const integerSquareRoot = (x) => {
  const result = Math.sqrt(x);
  const roundedResult = parseInt(result);
  if (roundedResult > currentSquareRoot) {
    currentSquareRoot = roundedResult;
  }
  if (result === roundedResult) {
    return true;
  } else {
    return false;
  }
};

const testIfPrime = (primeUnderTest) => {
  const hasSquareRoot = integerSquareRoot(primeUnderTest);
  if (hasSquareRoot) return false;

  for (const prime of primesArr) {
    if (prime >= currentSquareRoot) {
      primesArr.push(primeUnderTest);
      return true;
    }
    const remainder = primeUnderTest % prime;
    if (remainder === 0) {
      return false;
    }
  }
};

const generatePrimeNumbers = () => {
  let skip5 = 2;

  for (let i = 11; i < 100; i += 2) {
    if (skip5 === 4) {
      skip5 = 0;
    } else {
      skip5++;
      testIfPrime(i);
    }
  }
  console.log(JSON.stringify(primesArr));
  sumAll();
};

generatePrimeNumbers();
